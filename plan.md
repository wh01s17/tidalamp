...
| Ayuda del menú de la pista (`t`, `b`) | **VERIFICADO POR EL USUARIO**  | El mantenedor abrió `?` en la app (2026-09-14, en inglés): el menú de la pista lista `t` «go to the artist» y `b` «go to the album». Le falta `l` (añadir a una playlist), que el menú sí tiene; eso viene de antes de esta versión. |
